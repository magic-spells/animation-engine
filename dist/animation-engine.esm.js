...
var o = {
	...
	"back-in": (e) => e * e * (2.70158 * e - r),
	"back-out": (e) => {
		let t = e - 1;
		return t * t * (2.70158 * t + r) + 1;
	},
	"back-in-out": (e) => {
		let t = e * 2;
		if (t < 1) return .5 * (t * t * (3.5949095 * t - i));
		let n = t - 2;
		return .5 * (n * n * (3.5949095 * n + i) + 2);
	},
...
}, s = /^cubic-bezier\(\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^)]+)\)$/, c = !1;
function l(e) {
	if (typeof e == "function") return e;
	if (typeof e == "string") {
		let t = e.match(s);
		if (t) {
			let e = t.slice(1).map((e) => parseFloat(e)), [n, r, i, o] = e;
			if (e.every(Number.isFinite) && n >= 0 && n <= 1 && i >= 0 && i <= 1) return a(n, r, i, o);
		}
		if (o[e]) return o[e];
	}
	return c || (c = !0, console.warn(`AnimationEngine: unknown easing "${e}", falling back to linear.`)), o.linear;
}
//#endregion
...
function E(e, t) {
	...
	for (let i in t) {
		...
		if (i === "transform") {
			...
			x || (x = !0, console.warn("AnimationEngine: no tracked prior transform; first .to({transform}) will jump. Use .fromTo() or seed with .set() for a smooth first transform tween.")), r[i] = t[i];
			continue;
		}
...
var D = class {
	...
	start() {
		return v.get(this.el) !== this && y(this.el), v.set(this.el, this), this.duration <= 0 ? (this._lastStyles = this.endStyles, S(this.el, this.endStyles), this.onUpdate && this.onUpdate(this.endStyles, 1, this.el), this._finish(), this.promise) : (this._apply(0), this._onTick = (e) => this._tick(e), n.subscribe(this._onTick), this.promise);
	}
	_tick(e) {
		...
		if (t >= 1) {
			this._lastStyles = this.endStyles, S(this.el, this.endStyles), this.onUpdate && this.onUpdate(this.endStyles, 1, this.el), this._finish();
			return;
		}
		this._apply(t);
	}
	_apply(e) {
		...
		this._lastStyles = t, S(this.el, t), this.onUpdate && this.onUpdate(t, e, this.el);
	}
	_cleanup() {
		this._done = !0, this._onTick && n.unsubscribe(this._onTick), v.get(this.el) === this && v.delete(this.el);
	}
	_finish() {
		this._done || (this._cleanup(), b(this.el, this.endStyles), this._resolve());
	}
	cancel() {
		this._done || (this._cleanup(), this._lastStyles && b(this.el, this._lastStyles), this._resolve());
	}
}, O = null;
function k(e) {
	O = e;
...
//#endregion
//#region src/physics-tween.js
var j = class {
	constructor({ el: e, frameEngine: t, endStyles: n, physics: r, onUpdate: i }) {
		if (!A()) throw Error("Physics step requires registerPhysics(PhysicsEngine) — install @magic-spells/physics-engine and register it once.");
	...
	start() {
		return v.get(this.el) !== this && y(this.el), v.set(this.el, this), typeof requestAnimationFrame == "function" ? (this.engine = new (A())(this.physics), this._onChange = ({ progress: e }) => {
			this._done || this._apply(e);
		}, this.engine.on("change", this._onChange), this._apply(0), this.engine.animateTo(0, 100, 0).then(() => {
			this._done || (this._lastStyles = this.endStyles, S(this.el, this.endStyles), this.onUpdate && this.onUpdate(this.endStyles, 1, this.el), this._finish());
		}), this.promise) : (this._lastStyles = this.endStyles, S(this.el, this.endStyles), this.onUpdate && this.onUpdate(this.endStyles, 1, this.el), this._finish(), this.promise);
	}
	_apply(e) {
	...
	_cleanup() {
		this._done = !0, this.engine && this._onChange && this.engine.off("change", this._onChange), v.get(this.el) === this && v.delete(this.el);
	}
	_finish() {
		this._done || (this._cleanup(), b(this.el, this.endStyles), this._resolve());
	}
	cancel() {
		this._done || (this._cleanup(), this.engine && this.engine.stop(), this._lastStyles && b(this.el, this._lastStyles), this._resolve());
	}
}, M = 400, N = "ease";
function P() {
...
var L = class extends e {
	constructor(e = {}) {
		super(), this._steps = [], this._builder = F(this._steps), this._loop = e.loop ?? 1, this._loopDelay = e.loopDelay ?? 0, this._alternate = e.alternate ?? !1, this._defaults = e.defaults ?? {}, this._respectReducedMotion = e.respectReducedMotion ?? !0, this._onBegin = e.onBegin, this._onComplete = e.onComplete, this._onLoop = e.onLoop, this._timeScale = 1, this._playing = !1, this._current = null, this._playPromise = null;
	}
	to(e, t, n) {
	...
	async _execute(e) {
		if (this.emit("begin"), this._onBegin && this._onBegin(), this._respectReducedMotion && P()) {
	...
	async _runStep(e, t, n) {
		switch (e.type) {
			case "wait":
				await this._scaledWait(p(e.ms) || 0, n).promise;
				return;
			case "set":
				this._applySet(e);
				return;
	...
	_applySet(e) {
		w(e.target).forEach((t, n) => {
			let r = m(e.styles, t, n);
			y(t), S(t, r), b(t, r);
		});
	}
	async _runTween(e, t, n) {
		...
		let i = e.opts || {}, a = !!i.physics, o = p(i.delay) || 0, s = a ? 0 : p(i.duration) ?? p(this._defaults.duration) ?? M, c = l(i.easing ?? this._defaults.easing ?? N);
		if (o > 0 && (await this._scaledWait(o, n).promise, n.stopped || n.finished)) return;
		let u = r.map((r, o) => {
			let l = this._resolveStepValues(e, r, o);
			y(r);
			let { fe: u, endStyles: d } = this._buildFrameEngine(e, r, t, l), f = a ? new j({
				el: r,
				frameEngine: u,
				endStyles: d,
				physics: i.physics,
	...
	async _runStagger(e, t, n) {
		let r = w(e.targets);
		if (r.length === 0) return;
		let i = e.staggerOpts || {}, a = p(i.interval) || 0, o = i.jitter || 0, s = I(r.length, i.from), c = r.map((r, i) => this._runStaggerItem(e, r, i, s[i], a, o, t, n));
		await Promise.all(c);
	}
	async _runStaggerItem(e, t, n, r, i, a, o, s) {
		let c = typeof e.config == "function" ? e.config(t, n) : { ...e.config }, u = r * i + (p(c.delay) || 0);
		if (a && (u += (Math.random() * 2 - 1) * a * i), u < 0 && (u = 0), u > 0 && (await this._scaledWait(u, s).promise, s.stopped || s.finished)) return;
		let d = !!c.physics, f = d ? 0 : p(c.duration) ?? p(this._defaults.duration) ?? M, m = l(c.easing ?? this._defaults.easing ?? N);
		y(t);
		let { fe: h, endStyles: g } = this._buildFrameEngineFromConfig(c, t, o, n), _ = d ? new j({
			el: t,
			frameEngine: h,
			endStyles: g,
			physics: c.physics,
	...
	_resolveStepValues(e, t, n) {
		switch (e.type) {
			case "to":
			case "from": return { styles: m(e.styles, t, n) };
			case "fromTo": return {
				fromStyles: m(e.fromStyles, t, n),
				toStyles: m(e.toStyles, t, n)
			};
			case "frames": return { keyframes: g(h(e.keyframes, t, n)) };
			default: return {};
		}
	}
	_buildFrameEngine(e, n, r, i) {
		let a;
		if (e.type === "frames") a = r ? R(i.keyframes) : i.keyframes;
		else {
			let t, o;
	...
	_buildFrameEngineFromConfig(e, n, r, i) {
		let a;
		if (e.keyframes) {
			let t = g(h(e.keyframes, n, i));
			a = r ? R(t) : t;
		} else {
			let t = e.from ? m(e.from, n, i) : null, o = e.to ? m(e.to, n, i) : null;
			t && !o ? o = E(n, t) : !t && o ? t = E(n, o) : !t && !o && (t = {}, o = {}), r && ([t, o] = [o, t]), a = {
				0: t,
				100: o
			};
		}
		...
		switch (e.type) {
			...
			case "frames":
				w(e.target).forEach((t, n) => {
					...
					S(t, i), b(t, i), e.opts.onUpdate && e.opts.onUpdate(i, 1, t);
				});
				return;
			case "parallel":
			...
			case "stagger":
				w(e.targets).forEach((t, n) => {
					...
					S(t, i), b(t, i), r.onUpdate && r.onUpdate(i, 1, t);
				});
				return;
		}
	}
	_scaledWait(e, t) {
		let r = 0, i = !1, a, o = new Promise((e) => {
			a = e;
		}), s = () => {
			i || (i = !0, n.unsubscribe(c), t.active.delete(l), a());
		}, c = (t) => {
			i || (r += t * this._timeScale, r >= e && s());
		}, l = { cancel: s };
		return t.active.add(l), e <= 0 ? s() : n.subscribe(c), {
			promise: o,
			cancel: s
		};
	}
...
//#endregion
export { L as Scene, a as cubicBezier, o as easings, g as fillSparseKeyframes, f as pick, d as rand, k as registerPhysics, l as resolveEasing, z as scene, n as ticker };
